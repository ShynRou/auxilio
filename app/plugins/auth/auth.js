...
Auth.prototype = {
  ...
  login: async function (username, password, request) {
  ...
  register: async function (username, password, email, request) {
    if (this.users) {
  ...
  logout: async function (request) {
    let user = await this.sessions.findOne({_id: request.credentials._id});
